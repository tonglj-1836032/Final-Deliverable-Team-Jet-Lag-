...
age_case_p1 <- "Since many people have been unfortunately confirmed to be the
...
age_case_p2 <- "Based on this chart, we can observe that most of COVID-19
              ...
              chart, we can see that the majority of confirmed cases are
              founded in the age group above 30 years old, which might indicate
              that there is a positive correlation between the possibility of
              getting COVID-19 and the age of individuals."
...
server <- function(input, output) {
 ...
 reactive_data <- reactive({
   if (input$x == "sex") {
     plot_data <- data_race_sex
     }

   if (input$x == "age_group") {
     plot_data <- data_race_age
     }
 ...
 output$plot <- renderPlotly({
  ...
  for (i in 1:n) {
    ...
    for (j in 1:m) {
      dm = length(
        which(as.character(dd$sex) == sex[j] &
                as.character(dd$death_yn) == "Yes")) *
        100 / length(ss)
  ...
  max_F = res_F[max_F, 3]
  text = paste0(max_M, "% is the highest mortality rates for male, ", max_M_A,
              " is the age group with the highest mortality rate for male. "
              , max_F, "% is the highest mortality rates for female and ",
              max_F_A, " age group with the highest mortality rate for female.
              We could learn that before 75 years old, the male has higher
              mortality rates than female. However, after 75 years old,
              female has high mortality rates than male. And one interesting
              thing I found in this chart is that people who have 'other'
              gender identity has almost 0% mortality the rate in every age
              group.")
  ...
  plot_line <- ggplot(
    ...
    geom_line() +
    ggtitle("COVID-19 cases mortality rates for every sex in each age group.") +
    labs(
      y = "Mortality rates (%)",
      x = " Age group"
      )
